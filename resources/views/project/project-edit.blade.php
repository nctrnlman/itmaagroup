...
<form action="{{ route('projects.update', ['id' => $project->id_project]) }}" method="POST" id="createForm" enctype="multipart/form-data">
    @csrf
    <div class="row">
        <div class="col-lg-8">
            <div class="card">
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-4">
                            <div>
                                ...
                                <input type="text" class="form-control" id="datepicker-deadline-input" name="due_date"
                                    placeholder="Enter due date" data-provider="flatpickr" value="{{ $project->due_date ?? '' }}">
                            </div>
                        </div>
                    </div>
                </div>
...
<a href="javascript:void(0);" class="avatar-group-item" data-bs-toggle="tooltip" data-bs-trigger="hover"
    data-bs-placement="top" data-name="{{ $user->nama }}" >
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div>
                        ...
                        <button id="deleteFileButton" class="btn btn-sm btn-danger" style="display: none;">Delete File</button>
                        @if ($project->file)
...
<div class="search-box mb-3">
    <input type="text" class="form-control bg-light border-light search-input" for="choices-member-input" placeholder="Search here...">
    ...
    <div class="vstack gap-3"  data-choices id="choices-member-input">
        ...
        <div class="d-flex align-items-center member-item" data-name="{{ $user->nama }}">
            <div class="avatar-xs flex-shrink-0 me-3">
                @if($user->file_foto)
                <img src="{{ asset('uploads/uploads/'.$user->file_foto) }}" alt="" class="img-fluid rounded-circle">
                @else
                <img src="{{ asset('uploads/uploads/default.jpg') }}" alt="" class="img-fluid rounded-circle">
                @endif
            </div>
            <div class="flex-grow-1">
                <h5 class="fs-13 mb-0"><a href="#" class="text-body d-block" data-idnik="{{ $user->idnik }}">{{ $user->nama }}</a></h5>
            </div>
...
<script>
  ...
  for (var i = 0; i < addButtons.length; i++) {
    addButtons[i].addEventListener('click', function() {
      ...
      var memberIdnik = this.parentNode.parentNode.querySelector('.fs-13 a').getAttribute('data-idnik');
      ...
      for (var j = 0; j < existingMembers.length; j++) {
        var existingMemberInput = existingMembers[j].querySelector('input[name="selectedMembers[]"]');
  ...
  function updateMemberIds() {
    ...
    var selectedMembersInputs = teamMembersContainer.querySelectorAll('input[name="selectedMembers[]"]');
...
<script>var searchInput = document.querySelector('.search-box .search-input');
